        ...
        if (runtime == null) {
            val rt = GeckoRuntime.create(
                ...
                GeckoRuntimeSettings.Builder()
                    ...
                    // CRITICAL: this defaults to false. Without it, WebExtension install/list/
                    // uninstall and the toolbar action icon all keep working (that bookkeeping
                    // runs in the main process), but no extension process is ever spawned, so
                    // background scripts, content scripts, and webRequest listeners never run.
                    // uBlock Origin "installs" successfully but blocks nothing; Dark Reader
                    // "installs" but never repaints a page. Confirmed against Mozilla's own
                    // GeckoViewActivity.java reference, which always sets this explicitly.
